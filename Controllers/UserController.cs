...
public class UserController : ControllerBase
{
  ...
  signingCredentials: new SigningCredentials(new SymmetricSecurityKey(Encoding.UTF8.GetBytes(jwtSettings.securitykey)), SecurityAlgorithms.HmacSha256)
        ...
        return new TokenResponse() { jwttoken = jwttoken, refereshtoken = await refereshTokenGenerator.GenerateToken(user) };
    ...
    new Claim[] { new Claim(ClaimTypes.Name, user.Userid), new Claim(ClaimTypes.Role, user.Role) }
        ...
        var response = new TokenResponse() { jwttoken = finaltoken, refereshtoken = await refereshTokenGenerator.GenerateToken(userCred.username) };
    ...
    public async Task<IActionResult> RefToken([FromBody] TokenResponse tokenResponse)
    {
        ...
        var user = await this._DBContext.TblRefreshtokens.FirstOrDefaultAsync(item => item.UserId == username && item.RefreshToken == tokenResponse.refereshtoken);
        if (user == null)
            return Unauthorized();
